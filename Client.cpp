...
Client::Client(int clientFd) 
    : fd(clientFd), nickname(""), username(""), hostname(""), buffer(""), isRegistered(false), isOperator(false) {}
...
std::string Client::extractCommand() 
{
    ...
    // IRC standardı olan \r karakterini temizle
    if (!cmd.empty() && cmd[cmd.length() - 1] == '\r')
        cmd.erase(cmd.length() - 1);
    
    // İşlenen komutu buffer'dan sil
